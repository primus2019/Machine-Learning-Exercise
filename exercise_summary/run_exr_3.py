...
def showDataSet(dataMat, labelMat):
    ...
    plt.scatter(np.transpose(data_plus_np)[
                0], np.transpose(data_plus_np)[1])  # 正样本散点图
    plt.scatter(np.transpose(data_minus_np)[
                0], np.transpose(data_minus_np)[1])  # 负样本散点图
    plt.show()
...
def showClassifer(dataMat, labelMat, w, b):
    ...
    plt.scatter(np.transpose(data_plus_np)[0], np.transpose(
        data_plus_np)[1], s=30, alpha=0.7)  # 正样本散点图
    plt.scatter(np.transpose(data_minus_np)[0], np.transpose(
        data_minus_np)[1], s=30, alpha=0.7)  # 负样本散点图
    # 绘制直线
    x1 = max(dataMat)[0]
    x2 = min(dataMat)[0]
    a1, a2 = w
...
def get_w(dataMat, labelMat, alphas):
    ...
    w = np.dot((np.tile(labelMat.reshape(1, -1).T, (1, 10)) * dataMat).T, alphas)
    return w.tolist()


if __name__ == '__main__':
    # dataMat, labelMat = loadDataSet('testSet.txt')
    train_data, train_data_indices, train_data_cols = load_dataset(r'data\train_1_data.csv')
    train_target, train_target_indices, train_target_cols = load_dataset(r'data\train_1_target.csv')
    train_target = set_targets(train_target)
    train_data = scale(train_data, -1, 1)
    # info(train_data, train_data_indices, train_data_cols, train_target, False)
    # print(train_target.T.shape)
    b, alphas = smoSimple(train_data, train_target.T, 0.6, 0.15, 40)
    w = get_w(train_data, train_target.T, alphas)
    showClassifer(train_data, train_target.T, w, b)
